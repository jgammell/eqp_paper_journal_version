...
\noindent\textbf{Number of words:} 5137
...
{ \color{red} The main motivations for using equilibrium propagation instead of an alternative 
...
{\color{red} Just as in the algorithm, to implement equilibrium propagation in hardware, three
different phases of hardware operation are required. In the first (free running) phase, i}t follows
...
\ref{eqn:weight_correction} and \ref{eqn:bias_correction}. {\color{red} This is exactly the 
operation of an analog leaky integrate and fire neuron, as for example implemented in the 
neuromorphic hardware platforms of Refs. \citenum{Indiveri, BrainScaleS} among others. A 
qualitative diagram of potential neuron and synapse devices and their output and read/write to 
memory operations are shown in the diagram in Fig. \ref{fig:eq_prop} (a). At each neuron $N^l_{i}$ 
...
generate $U_{l+1}$. A block diagram showing the same thing in more detail is shown in the Appendix.

In the second (weakly-clamped) phase, shown in Fig. \ref{fig:eq_prop} (b), the operation of the 
hardware is exactly the same as in (a), with the value $U_{\beta}$ written to memory at each 
neuron. Not shown in Fig. \ref{fig:eqp_bd} is the functionality at the output neurons which are 
weakly clamped and have a new function in this phase, this is shown in Fig. X in the Appendix. 

Finally, in the third phase, the weights and biases are updated as shown in Fig. \ref{fig:eq_prop}
(c). At each neuron device the values of $U_0$ and $U_\beta$ are read from memory and $\rho({U_0})$
and $\rho({U_\beta})$ are calculated. At the synapse device, the computation of Eq. X is performed 
using these values from the pre- and post- synaptic neurons to calculate the weight update $\Delta 
w$, and the value of the weight in memory is updated to $w+\Delta w$.  Similar updates are applied 
to the bias $b$ at every neuron according to Eq. Y}. {\color{blue} Brief sentence on number of 
...
{\color{red} Backpropagation is an algorithm for training networks using gradient descent. It is 
most typically applied to feedforward neural networks, in which } the activation value of a neuron 
$i$ in layer $l$ is given by $$\rho(u_i^l)=\rho(\sum_jW_{ij}^lu_j^{l-1}+b_i^l).$$ {\color{red} This
...
phase in hardware is shown in Fig. \ref{fig:back_prop} (a). Using backpropagation, t}he 
...
$$\delta_i^l=\rho'(u_i^l)\sum_jW_{ij}^{l+1}\delta_j^{l+1}.$$ {\color{red} The implementation of 
this in hardware is shown in Fig. \ref{fig:back_prop} (b) (excluding layer $L$). Note that the data
...
$$\Delta b_i^l=\delta_i^l.$$ {\color{red} This is shown in Fig. \ref{fig:back_prop} (c)}.
...
requires only slight modification to output neurons, whereas in backpropagation these phases very 
different functionality from essentially all neurons. {\color{blue} not sure I understand this next
sentence}{\color{green}Another visible difference is that in equilibrium propagation each neuron 
corresponds to a single synapse whereas in backpropagation a neuron corresponds to two synapses; we
do not expect this difference to be significant because a synapse in the former case a synapse 
takes as inputs the outputs of all neighboring neurons, whereas in the latter case each has inputs 
from either shallower or deeper neurons (about half as many)}. {\color{red} Neurons in equilibrium 
propagation explicitly write their states to memory after the free and weakly-clamped phases, and 
therefore it requires the same amount of memory as for backpropagation}. In backpropagation, 
distinct state variables are needed to hold the activation and error term of each neuron. 
{\color{red} For a hardware implementation, the need for distinct free and weakly-clamped phases 
...
algorithm can be modified to eliminate the need for both phases \cite{NewEquilibriumPropPaper}}. 
